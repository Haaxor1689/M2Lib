...
namespace M2Lib.m2
{
    ...
    public static class AnimationData
    {
        ...
        static AnimationData()
        {
            ...
            var embeddedStream =
                assembly.GetManifestResourceStream("M2Lib.src.csv.AnimationData.csv")
                ?? throw new Exception("Could not open embedded resource AnimationData");

            var csvParser = new TextFieldParser(embeddedStream) { CommentTokens = ["#"] };
            ...
            foreach (var value in playThenStopValues)
                PlayThenStop.Add(value);
            ...
            foreach (var value in playBackwardsValues)
                PlayBackwards.Add(value);
