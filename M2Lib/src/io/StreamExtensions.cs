    ...
    public static class StreamExtensions
    {
            ...
            where T : new()
        {
            if (!typeof(IMarshalable).IsAssignableFrom(typeof(T)))
                return (T)ReadFunctions[typeof(T)](stream);
            var item = new T();
            ((IMarshalable)item).Load(stream, version);
            return item;
        }

        public static void WriteGeneric<T>(this BinaryWriter stream, M2.Format version, T item)
            where T : new()
        {
            if (typeof(IMarshalable).IsAssignableFrom(typeof(T)))
                ((IMarshalable)item).Save(stream, version);
            else
                WriteFunctions[typeof(T)](stream, item);
        }

        //READING OF STRUCTS
        public static C2Vector ReadC2Vector(this BinaryReader stream) =>
            new(stream.ReadSingle(), stream.ReadSingle());

        public static C33Matrix ReadC33Matrix(this BinaryReader stream) =>
            new(stream.ReadC3Vector(), stream.ReadC3Vector(), stream.ReadC3Vector());

        public static C3Vector ReadC3Vector(this BinaryReader stream) =>
            new(stream.ReadSingle(), stream.ReadSingle(), stream.ReadSingle());
